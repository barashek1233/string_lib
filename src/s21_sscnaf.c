...
int s21_sscanf(const char *str, const char *fstr, ...) {
  ...
  if (!eof_fl) {
    ...
    while (*formstr && formstr < fstr + len) {
      tokens[tokens_len] = get_tokens(&formstr, &va);
      tokens_len++;
    }
...
token get_tokens(char **fstr, va_list *va) {
...
void skip_chars_in_buffer(char **src, int *fail, token *tok) {
  ...
  int test = s21_strspn(*src, tok->buffer);
...
void fstr_get_width(char **fstr, token *tok) {
  if (**fstr == '*') {
    (*fstr)++;
    tok->width = WIDTH_AST;
  } else {
    int res = get_number_from_fstr(fstr);
    if (res) {
...
int get_number_from_fstr(char **fstr) {
...
void fstr_get_length(char **fstr, token *tok) {
  switch (**fstr) {
    case 'h':
...
void fstr_get_specifier(char **fstr, token *tok) {
  tok->spec = (**fstr);
  (*fstr)++;
}
  ...
  *fail_flag = 1;
  ...
  if (s21_strspn(*str, "0123456789+-")) {
    ...
    if (!(sign > 1 || (sign && (tok->width_n <= 1 && tok->width)))) {
      buffer[0] = **str;
      (*str)++;
      write_chars_to_buffer(str, "0123456789", buffer, tok->width_n, 1);
  ...
  if (tok->width != WIDTH_NUMBER)
    write_chars_to_buffer(str, "0123456789", S21_NULL, 0, 0);
}
...
void write_float_to_memory(char **str, int *res, token *tok) {
  ...
  if (test) {
    ...
    if (!(sign > 1 || (sign && (tok->width_n <= 1 && tok->width)))) {
      ...
      if (tok->spec == 'f')
        write_chars_to_buffer(str, ".0123456789+-", buffer, tok->width_n,
                              start_ind);
      else
        write_chars_to_buffer(str, ".0123456789eE+-NnaAifIF", buffer,
  ...
  if (tok->width != WIDTH_NUMBER) {
    if (tok->spec == 'f')
      ...
      write_chars_to_buffer(str, ".0123456789eE+-NaAifIFn", S21_NULL, 0, 0);
  }
}
...
void write_string_to_memory(char **str, const int *fail_flag, int *res,
  ...
  if (tok->width != WIDTH_AST && succ) {
    s21_strcpy((char *)tok->addr, buffer);
    (*res)++;
  }
}
